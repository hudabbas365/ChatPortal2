...
public sealed class FileDatasourceService : IFileDatasourceService
{
    ...
    public async Task<(bool Success, string? Error)> TestAsync(string url)
    {
        ...
        try
        {
            ...
            // downloading the full file. Fall back to GET with zero-byte read if
            ...
            catch { headResponse = new HttpResponseMessage(System.Net.HttpStatusCode.MethodNotAllowed); }
            ...
            {
                // Server doesn't support HEAD — fetch with GET but read just one byte
    ...
    public async Task<QueryExecutionResult> ExecuteAsync(Datasource ds, int maxRows = 1000)
    {
        ...
        string normalized;
        try { normalized = NormalizeUrl(rawUrl); }
    ...
    internal static string NormalizeUrl(string url)
    {
        ...
        // Cannot be resolved server-side without following the redirect, so we
        // leave as-is and rely on HttpClient following it. The HEAD probe in
        // TestAsync will surface 401/403 if the link is not "Anyone with the link".
        ...
        {
            if (!uri.Query.Contains("download=1", StringComparison.OrdinalIgnoreCase))
            {
                var q = System.Web.HttpUtility.ParseQueryString(uri.Query);
                q["download"] = "1";
